...
//    f = fopen(fx->path,"r");
//    if(!f){
    fd = SPIFFS_open(&fs, fx->path, SPIFFS_O_RDONLY, 0);
    if (fd < 0) {
      fx->valid = false;
...
//    fread(buf, sizeof buf, 1, fx->file);
    if (SPIFFS_read(&fs, fx->fd, buf, sizeof(buf)) != sizeof(buf)) {
      fx->valid = false;
...
if(FontxDebug)printf("[GetFontx]offset=%d\n",offset);
        ret = SPIFFS_lseek(&fs, fxs[i].fd, offset, SPIFFS_SEEK_SET);
if(FontxDebug)printf("[GetFontx]lseek ret=%d\n",ret);
        if (ret != offset) {
          printf("Fontx:seek(%u) failed.\n",offset);
          return false;
        }
        if (SPIFFS_read(&fs, fxs[i].fd, pGlyph, fxs[i].fsz) != fxs[i].fsz) {
...
if(FontxDebug)printf("[GetFontx]lseek ret=%d\n",ret);
        ...
        while(bc--){ 
          if (SPIFFS_read(&fs, fxs[i].fd, (char *)buf, sizeof(buf)) != sizeof(buf)) {
            printf("Fontx:fread failed.\n");
            return false;
          }
